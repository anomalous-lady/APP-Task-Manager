...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget _buildBody(TasksState state, List filteredTasks) {
    ...
    if (state.error != null && state.tasks.isEmpty) {
      return ErrorState(
        message: state.error!,
        onRetry: () => ref.read(tasksProvider.notifier).loadTasks(refresh: true),
      );
    }
    ...
    return RefreshIndicator(
      onRefresh: () => ref.read(tasksProvider.notifier).loadTasks(refresh: true),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _Divider(),
          _StatItem(label: 'Pending', value: pending, color: Colors.orange.shade200),
          _Divider(),
          _StatItem(label: 'Done', value: completed, color: Colors.greenAccent.shade200),
        ],
      ),
...
class _Divider extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(height: 30, width: 1, color: Colors.white30);
}
